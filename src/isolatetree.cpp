...
std::vector<pcl::PointCloud<pcl::PointXYZ>::Ptr> getslices(pcl::PointCloud<pcl::PointXYZ>::Ptr plot, pcl::PointCloud<pcl::PointXYZ>::Ptr stem, float step, float extent)
{
	Eigen::Vector4f min,max,centroid;
	pcl::compute3DCentroid(*stem,centroid);
//	std::cout << centroid << std::endl;
	pcl::getMinMax3D(*stem,min,max);
	cylParams cyl = fitCylinder(stem,60);
//	float extent = maxcrown(cyl.rad*2);
	float height = maxheight(cyl.rad*2);
	std::cout << cyl.rad * 2 << " " << extent << " " << height << std::endl;
	pcl::PointCloud<pcl::PointXYZ>::Ptr volume(new pcl::PointCloud<pcl::PointXYZ>);
	pcl::PassThrough<pcl::PointXYZ> pass;
	pass.setInputCloud(plot);
	pass.setFilterFieldName("x");
	pass.setFilterLimits(centroid[0]-extent/2,centroid[0]+extent/2);
	pass.filter(*volume);
	pass.setInputCloud(volume);
	pass.setFilterFieldName("y");
	pass.setFilterLimits(centroid[1]-extent/2,centroid[1]+extent/2);
	pass.filter(*volume);
	pass.setInputCloud(volume);
	pass.setFilterFieldName("z");
	pass.setFilterLimits(max[2],min[2]+height);
	pass.filter(*volume);
	*volume += *stem;
...
//void pruneclusters(std::vector<pcl::PointCloud<pcl::PointXYZ>::Ptr> &clusters, float sphericity_max, float eigenratio_a_max, float eigenratio_b_min)
//{
//
...
void buildtree(float maxdistance, pcl::PointCloud<pcl::PointXYZ>::Ptr stem, std::vector<pcl::PointCloud<pcl::PointXYZ>::Ptr> &clusters, pcl::PointCloud<pcl::PointXYZ>::Ptr &t)
{
	...
	while(done_something == true)
	{
		...
		for(int i=0;i<outer.size();i++)
		{
			...
			float distance_max = maxdistance; 
...
//	int ntiles= atoi(argv[3]) + 4;
//	for(int i=4;i<ntiles;i++)
//	{
	...
	for(int i=7;i<argc;i++)
	{
		...
		if ( !boost::filesystem::exists(ss.str()) )
		{
			if ( !boost::filesystem::exists(ll) )
			{
				...
				{
					float expansion = atof(argv[6]);
					if(expansion < 15.0)
					{
						std::cout << "Updating search radius to 15m" << std::endl;						
						expansion = 15.0;
					}
		...
		//    			std::cout << *it << std::endl;
					std::stringstream ss;
					ss.str("");
					ss << argv[3] << argv[4] << "_" << *it << ".downsample.pcd";
					pcl::PointCloud<pcl::PointXYZ>::Ptr cloud_tmp(new pcl::PointCloud<pcl::PointXYZ>);		
				...
				std::vector<pcl::PointCloud<pcl::PointXYZ>::Ptr> clusters;
				float step = 1;
				float extent = atof(argv[6]) * 2;
				clusters = getslices(plot,stem,step,extent);
				ss.str("");
				ss << "slices_" << fname << ".pcd";
				...
				ss.str("");
				ss << "slices_clusters_corrected_" << fname << ".pcd";
				writeclusters(clusters,ss.str(),false);
				std::cout << ss.str() << std::endl;
				// This section prunes the leaves
		//		std::cout << "Cluster pruning: " << std::flush;
		//		float sphericity_max = atof(argv[6]); 
		//		float eigenratio_a_max = atof(argv[7]);
		//		float eigenratio_b_min = atof(argv[8]);
		//		pruneclusters(clusters,sphericity_max,eigenratio_a_max,eigenratio_b_min);
		//		ss.str("");
		//		ss << "slices_clusters_corrected_pruned_" << fname << ".pcd";
		//		writeclusters(clusters,ss.str(),false);
		//		std::cout << ss.str() << std::endl;
		//		//
				std::cout << "Building tree: " << std::flush;
				pcl::PointCloud<pcl::PointXYZ>::Ptr tree(new pcl::PointCloud<pcl::PointXYZ>);
				float maxdistance = atof(argv[1]);
				buildtree(maxdistance,stem,clusters,tree);
				ss.str("");
				ss << pname << "_" << fname << ".pcd";
