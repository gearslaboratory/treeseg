...
// Modified by Kim Calders: read in multiple cluster files (e.g. slice_clusters_corrected_fitted_principle_concatenated.pcd), merge them and divide them into clusters again
...
int main (int argc, char** argv)
{
	...
	std::cout << "Read in data: " << std::flush;
	for(int i=3;i<argc;i++)
	{
		pcl::PointCloud<pcl::PointXYZ>::Ptr cloud_tmp(new pcl::PointCloud<pcl::PointXYZ>);
		reader.read(argv[i],*cloud_tmp);
		std::cout << argv[i] << std::endl;
		*plot_cloud += *cloud_tmp;
	}
	std::stringstream ss;
	//
	std::cout << "Cluster extraction: " << std::flush;
	std::vector<pcl::PointCloud<pcl::PointXYZ>::Ptr> clusters;
	int NSearch = atof(argv[1]); //1.5x voxel neighbours
	int min_count = atof(argv[2]);
