...
//		reader.read(argv[i],*tree);
		...
		float z = 1.3 - (z_step / 2); //i.e. first slice centre @ 1.3
		bool stable = false;
		while(stable != true)
		{
			...
			pass.setFilterLimits(min[2]+z,min[2]+z+z_step);
			pass.filter(*slice);
			pass.setInputCloud(tree);
			pass.setFilterFieldName("z");
			pass.setFilterLimits(min[2]+z+z_step,min[2]+z+z_step+z_step);
			pass.filter(*slice_forward);
			cylParams cyl,cyl_forward;
			cyl = fitCylinder(slice,NSearch);
			cyl_forward = fitCylinder(slice_forward,NSearch);
			float delta_rad = 1-(std::min(cyl.rad,cyl_forward.rad)/std::max(cyl.rad,cyl_forward.rad));
			//std::cout << cyl.rad * 2 << " " << cyl.stepcov << " " << cyl.radratio << " " << delta_rad << std::endl;
			if(delta_rad < 0.1)
			{
				x = cyl.x;
				y = cyl.y;
		...
		std::cout << fname << " " << x << " " << y << " " << dbh << " " << h << std::endl;
