...
std::vector<pcl::PointCloud<pcl::PointXYZ>::Ptr> getslices(pcl::PointCloud<pcl::PointXYZ>::Ptr plot, pcl::PointCloud<pcl::PointXYZ>::Ptr stem, float step)
{
	Eigen::Vector4f min,max,centroid;
...
//	reader.read(argv[1],*plot);
	for(int i=ntiles;i<argc;i++)
	{
		...
		std::vector<pcl::PointCloud<pcl::PointXYZ>::Ptr> clusters;
		float step = 1;
		clusters = getslices(plot,stem,step);
		ss.str("");
